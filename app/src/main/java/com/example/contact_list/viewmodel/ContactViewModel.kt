/* CLASSE POUR GÉRER L'ÉTAT DE LA LISTE DE CONTACTS */
...
class ContactViewModel : ViewModel() {
    ...
    private var uiState by mutableStateOf(
        ContactUiState(contacts = DataSource.loadData().sortedWith(compareBy { it.prenom.lowercase() }))
    )
    ...
    fun updateList(newList: List<Contact>) {
        uiState = uiState.copy(contacts = newList.sortedWith(compareBy { it.prenom.lowercase() }))
    }
